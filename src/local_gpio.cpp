...
//
//      $Id: local_gpio.cpp,v 1.1 2014/03/27 16:25:24 cvs Exp $
//
...
LocalGpio::LocalGpio(int id,QObject *parent,const char *name)
  : QObject(parent,name)
{
  Clear();
  gpio_id=id;
...
QString LocalGpio::device() const
{
  return gpio_device;
}


void LocalGpio::setDevice(QString dev)
{
  gpio_device=dev;
}


LocalGpio::Transition LocalGpio::transition(int line) const
{
  return gpio_transitions[line];
}


void LocalGpio::setTransition(int line,LocalGpio::Transition trans)
{
  gpio_transitions[line]=trans;
}


QString LocalGpio::description() const
{
  return gpio_description;
}


LocalGpio::Mode LocalGpio::mode()
{
  struct gpio_info info;

  switch(gpio_api) {
      case LocalGpio::ApiGpio:
	ioctl(gpio_fd,GPIO_GETINFO,&info);
	return (LocalGpio::Mode)info.mode;

      case LocalGpio::ApiInput:
	return LocalGpio::Auto;
  }
  return LocalGpio::Auto;
}


void LocalGpio::setMode(LocalGpio::Mode mode)
{
...
bool LocalGpio::open()
{
  ...
  if(ioctl(gpio_fd,GPIO_GETINFO,&gpio_info)==0) {
    gpio_api=LocalGpio::ApiGpio;
    InitGpio();
    RemapTimers();
  }
  else {
    if(ioctl(gpio_fd,EVIOCGVERSION,&ver)==0) {
      gpio_api=LocalGpio::ApiInput;
      InitInput();
    }
    else {
      ::close(gpio_fd);
...
bool LocalGpio::inputState(int line)
{
  if(!gpio_open) {
    return false;
  }
...
unsigned LocalGpio::outputMask() const
{
  struct gpio_mask mask;
...
void LocalGpio::gpoSet(int line,unsigned interval)
{
...
void LocalGpio::gpoReset(int line,unsigned interval)
{
  struct gpio_line out;
...
void LocalGpio::inputTimerData()
{
  ...
  if((input_mask=inputMask())!=gpio_input_mask) {
	...
	if((input_mask&mask)==0) {
	  emit inputChanged(gpio_id,i,false);
	}
	else {
	  emit inputChanged(gpio_id,i,true);
	}
      }
    }
  ...
  if((output_mask=outputMask())!=gpio_output_mask) {
	...
	if((output_mask&mask)==0) {
	  emit outputChanged(gpio_id,i,false);
	}
	else {
	  emit outputChanged(gpio_id,i,true);
	}
      }
    }
  ...
  void LocalGpio::revertData(int id)
{
...
void LocalGpio::SetReversion(int line,unsigned interval)
{
  if(interval==0) {
...
void LocalGpio::Clear()
{
  ...
  for(int i=0;i<GPIO_MAX_LINES;i++) {
    gpio_transitions[i]=LocalGpio::TransRising;
  }
}


void LocalGpio::InitGpio()
{
